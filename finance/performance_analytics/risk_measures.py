...
def active_premium(Rp, Rb, scale=MONTHLY):
    ...
    .. note::
       the methodology used here relies on pairwise comparison of each
       'portfolio' returns series to each 'benchmark' return series;
       reason being, that `annualised return` [whilst sharing the same
       frequency/scale] should be directly comparable where the series
       are of different lengths such that, where Rp[A, B, C] vs Rb[X, Y].
    ...
    def calc(df):
        ...
        portfolio and benchmark series) is simply to use the dropna()
        function on the combined set of data.
        '''
        df = df.dropna()
...
def tracking_error(Rp, Rb, scale=MONTHLY):
...
def information_ratio(Rp, Rb, scale=MONTHLY):
    '''relates the *degree* to which an investment has beaten the
    ...
    ap = active_premium(Rp, Rb, scale=scale)
    te = tracking_error(Rp, Rb, scale=scale)
